...
if len(number) == 6:
    start_num = int(number[0]) + int(number[1]) + int(number[2])
    end_num = int(number[3]) + int(number[4]) + int(number[5])
    if start_num == end_num:
        print(f"Билет под номером {number} счастливый!!!")
    else:
        print(f"Билет под номером {number} не счастливый!!!")
else:
    print(f"Билет под номером {number} не шестизначный")